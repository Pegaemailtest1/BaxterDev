...
def prompt():
    ...
    full_doc_search = payload.get("full_doc_search", False)
    if not user_query:
        return jsonify({"response": "Oops! Type something first, love!"}), 400
    response = requests.post(f"{FASTAPI_URL}/prompt", json={"prompt_query": prompt_query, "full_doc_search": full_doc_search, "user_query": user_query})
    response_data = response.json()
    return jsonify(response_data)
