...
def retrieve_content(config, query_text):
    ...
    document_filter_value= "I need to be able to see the set through the packaging."
    where_filter = "{\"document_id\": {\"$eq\": \"BXU535427 Rev D_Design Input - Requirements\"}}"
    where_document = f'{{"$contains": "{document_filter_value}"}}'
    ...
    generate_text_file(f"documents.txt",documents)
    #logging.info(f"metadatas retrieved from chromadb: {metadatas}")
    generate_text_file(f"metadatas.txt",metadatas)
