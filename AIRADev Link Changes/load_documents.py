...
from .extract_excel_chunks import extract_excel_chunks
...
def extract_pdf_text_tables_images(pdf_path):
    ...
    # Load reference icon map
    #with open("reference_icon_map.json", "r") as f: reference_icon_map = json.load(f)
    reference_icon_map_path = os.path.join(
        os.path.dirname(__file__),  # this will resolve to documents_ingestion/
        "reference_icon_map.json"
    )

    with open(reference_icon_map_path, "r", encoding="utf-8") as f:
        reference_icon_map = json.load(f)
    ...
    for page_index in range(len(pdf_doc)):
        ...
        for img_index, img in enumerate(images):
            ...
            for rect in image_rects:
                ...
                icon_hash = hash_image(image_bytes)
                ocr_text = ""
                icon_description = ""

                # Compare with reference icon hash
                match = reference_icon_map.get(icon_hash)
                if match:
                    icon_description = match["description_en"]
                ...
                extracted_content.append({
                    "type": "matched-icon" if match else ("header-image" if is_header else "image"),
                    "page": page_index + 1,
                    "format": image_ext,
                    "match": bool(match),
                    "icon_label": match["label"] if match else None,
                    "description": icon_description or ocr_text,
                    "content": f"[Image {img_index+1} on Page {page_index+1}]\n{icon_description or ocr_text}",
            ...
            try:
                ...
                if file.lower().endswith(".pdf"):
                    ...
                    for entry in full_text:
                        ...
                        if "section" in entry:
                            base_metadata["section"] = entry["section"]
                        if "normalized_section" in entry:
                            base_metadata["normalized_section"] = entry["normalized_section"]
                        if entry["type"] == "matched-icon":
                            base_metadata["icon_label"] = entry.get("icon_label")
                            base_metadata["description"] = entry.get("description")
                    ...
                    excel_chunks = extract_excel_chunks(full_path)  # This must return list of chunks
                    for idx, chunk in enumerate(excel_chunks):
                        chunk_metadata = {
                            "source": full_path,
                            "document_id": document_id,
                            "type": "excel",
                            "sheet": chunk.get("sheet", "Unknown"),
                            "row_range": chunk.get("row_range", f"chunk_{idx+1}")
                        }
                        all_docs.append(LangChainDoc(
